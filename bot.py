...
BOT_TOKEN = "8771472757:AAFM5l416Luw340V1GUTz9oq1sjfPwyJiu0"
OWNER_ID = 47230981
...
# ============================================================
# كلاس DeleteManager لإدارة حذف الرسائل المستقبلية
# ============================================================

class DeleteManager:
    """
    كلاس مسؤول عن تتبع وحذف الرسائل التي يرسلها المستخدم بعد أمر معين
    """
    
    def __init__(self, client, source_enabled_func, is_my_message_func, safe_edit_func, user_id):
        """
        تهيئة كلاس إدارة الحذف
        ...
        # متغيرات التتبع والحذف
        self.scheduled_delete_task = None
        self.scheduled_delete_time = None
        self.scheduled_delete_chat = None
        self.tracking_messages = False
    ...
    async def schedule_delete_future_messages(self, e, seconds):
        """
        جدولة حذف جميع الرسائل التي سيرسلها المستخدم بعد هذا الأمر
        
        المعاملات:
            e: كائن الحدث
            seconds: عدد الثواني للانتظار قبل الحذف
        """
        try:
            seconds = int(seconds)
            if seconds <= 0:
                await self.safe_edit(e, "✧ الـوقـت لازم يـكـون اكـبـر مـن 0")
                return
            
            if seconds > 3600:  # حد أقصى ساعة
                await self.safe_edit(e, "✧ الـحـد الاقـصـى 3600 ثـانـيـه (سـاعـه)")
                return
            ...
            # مسح القائمة السابقة
            self.messages_to_delete = []
            self.tracking_messages = True
            
            # حفظ معلومات الجدولة
            self.scheduled_delete_chat = chat_id
            self.scheduled_delete_time = time.time() + seconds
            ...
            confirm_msg = await self.client.send_message(
                chat_id,
                f"ᯓ『جـاري تـتـبـع رسـائـلك لـمـدة {seconds} ثـانـيـه سـيـتـم حـذفـهـا بـعـدهـا』ᯓ"
            )
            
            # إضافة رسالة التأكيد إلى قائمة الحذف
            self.messages_to_delete.append(confirm_msg.id)
            
            # بدء المهمة
            if self.scheduled_delete_task and not self.scheduled_delete_task.done():
                self.scheduled_delete_task.cancel()
            
            self.scheduled_delete_task = asyncio.create_task(
                self._delete_future_messages(chat_id, seconds, confirm_msg)
            )
            
        except ValueError:
            await self.safe_edit(e, "✧ الـرقـم غـلـط")
        except Exception as ex:
            await self.safe_edit(e, f"✧ خـطـأ: {str(ex)}")
    
    async def _delete_future_messages(self, chat_id, seconds, confirm_msg):
        """
        تنفيذ الحذف بعد الوقت المحدد
        
        المعاملات:
            chat_id: معرف المجموعة
            seconds: عدد الثواني للانتظار
            confirm_msg: رسالة التأكيد المرسلة
        """
        try:
            # انتظار الوقت المحدد
            ...
            # نسخ قائمة الرسائل للحذف
            messages_to_delete = self.messages_to_delete.copy()
            ...
            for i in range(0, len(messages_to_delete), batch_size):
                ...
                try:
                    ...
                    print(f"[DeleteManager] خطأ في حذف دفعة: {ex}")
            
            # حذف رسالة التأكيد إذا كانت موجودة
            try:
            ...
            try:
                await self.client.send_message(
                    chat_id,
                    f"ᯓ『تـم حـذف {deleted_count} رسـالـه ارسـلـتهـا خـلال الـفـتـرة』ᯓ"
                )
            except:
                pass
    ...
    async def cancel_future_delete(self, e):
        """
        إلغاء جدولة الحذف
        ...
        المعاملات:
            e: كائن الحدث
        """
        if not self.scheduled_delete_task or self.scheduled_delete_task.done():
            await self.safe_edit(e, "✧ مـفـيـش جـدولـة حـذف")
            return
        ...
        self.scheduled_delete_task.cancel()
        try:
            await self.scheduled_delete_task
        except asyncio.CancelledError:
            pass
        ...
        await self.safe_edit(e, "ᯓ『تـم الـغـاء جـدولـة الـحـذف』ᯓ")
    
    async def future_delete_status(self, e):
        """
        عرض حالة الجدولة
        ...
        remaining = int(self.scheduled_delete_time - time.time())
        if remaining < 0:
            remaining = 0
        ...
        await self.safe_edit(
            e,
            f"ᯓ『مـتـبـقـي {minutes} د {seconds_remaining} ث عـلـى الـحـذف - رسـائـل مـتـبـعـة: {len(self.messages_to_delete)}』ᯓ"
        )
    
    async def track_new_message(self, e):
        """
        تتبع الرسائل الجديدة التي يرسلها المستخدم خلال فترة الجدولة
        
        المعاملات:
            e: كائن الحدث
        """
        # التحقق من أن التتبع مفعل وهناك جدولة
        if not self.tracking_messages:
            return
        ...
        # التأكد من أن الرسالة في نفس المجموعة
        if e.chat_id != self.scheduled_delete_chat:
            return
        
        # تجاهل الأوامر (التي تبدأ بـ .)
        if e.raw_text and e.raw_text.startswith('.'):
            return
        
        # إضافة معرف الرسالة إلى قائمة الحذف
        if e.id not in self.messages_to_delete:
            self.messages_to_delete.append(e.id)
    
    async def stop_all(self):
        """
        إيقاف جميع مهام الحذف (يستخدم عند إيقاف الجلسة)
        """
        try:
            if self.scheduled_delete_task and not self.scheduled_delete_task.done():
                self.tracking_messages = False
            ...
            self.messages_to_delete = []
            self.tracking_messages = False
            print("[DeleteManager] تم إيقاف جميع مهام الحذف")
        except Exception as ex:
            print(f"[DeleteManager] خطأ في إيقاف المهام: {ex}")
    
    def _setup_handlers(self):
        """
        تسجيل جميع معالجات الأوامر الخاصة بحذف الرسائل
        """
        
        @self.client.on(events.NewMessage(pattern=r'^\.مسح\s+(\d+)$', outgoing=True))
        async def delete_future_cmd(e):
            if not self._check_access(e):
                return
            
            seconds = e.pattern_match.group(1)
            await self.schedule_delete_future_messages(e, seconds)
        
        @self.client.on(events.NewMessage(pattern=r'^\.الغاء المسح$', outgoing=True))
        async def cancel_delete_cmd(e):
            if not self._check_access(e):
                return
            
            await self.cancel_future_delete(e)
        
        @self.client.on(events.NewMessage(pattern=r'^\.حالة المسح$', outgoing=True))
        async def delete_status_cmd(e):
            if not self._check_access(e):
                return
            
            await self.future_delete_status(e)
        
        @self.client.on(events.NewMessage(pattern=r'^\.م18$', outgoing=True))
        async def delete_menu(e):
            if not self._check_access(e):
                return
            
            text = f"""
⟪───── اوامـر الـمـسـح الـتـلـقـائـي ─────⟫

│`.مسح + الثواني` ↤ يمسح كل رسائلك الجديدة بعد الوقت المحدد
│`.الغاء المسح` ↤ يلغي جدولة الحذف
│`.حالة المسح` ↤ يعرض الوقت المتبقي وعدد الرسائل

╰─────────────────────────
"""
            await self.safe_edit(e, text)
        
        # معالج تتبع الرسائل الجديدة
        @self.client.on(events.NewMessage(outgoing=True))
        async def track_new_messages_handler(e):
            if not self.source_enabled_func():
                return
            if self.user_id in banned_users:
                return
            
            await self.track_new_message(e)

# ============================================================
# كلاس VoiceChanger لتغيير الصوت في الرسائل الصوتية والفيديو نوت
# ============================================================
...
class VoiceChanger:
    def __init__(self, client, source_enabled_func, is_my_message_func, safe_edit_func, user_id):
        ...
        self.effects = {
            "chipmunk": {"name": "صوت عالي", "cmd": ".صوت عالي", "ffmpeg": "atempo=1.5,asetrate=48000*1.5"},
            "deep": {"name": "صوت عميق", "cmd": ".صوت عميق", "ffmpeg": "atempo=0.8,asetrate=48000*0.7"},
            "robot": {"name": "روبوت", "cmd": ".روبوت", "ffmpeg": "aecho=0.8:0.9:1000:0.3,acrusher=0.1:0.1:0.1"},
            "echo": {"name": "صدى", "cmd": ".صدى", "ffmpeg": "aecho=0.8:0.9:500:0.3,aecho=0.8:0.9:1000:0.2"},
            "slow": {"name": "بطيء", "cmd": ".بطيء", "ffmpeg": "atempo=0.7"},
            "fast": {"name": "سريع", "cmd": ".سريع", "ffmpeg": "atempo=1.5"},
            "helium": {"name": "هيليوم", "cmd": ".هيليوم", "ffmpeg": "atempo=1.3,asetrate=48000*1.3"},
            "demon": {"name": "شيطان", "cmd": ".شيطان", "ffmpeg": "atempo=0.7,asetrate=48000*0.5"},
            "whisper": {"name": "همس", "cmd": ".همس", "ffmpeg": "volume=0.3,aecho=0.8:0.9:500:0.1,aecho=0.8:0.9:1000:0.1"},
            "alien": {"name": "فضائي", "cmd": ".فضائي", "ffmpeg": "aecho=0.8:0.9:500:0.3,atempo=1.2,asetrate=48000*0.9"},
            "chorus": {"name": "كورس", "cmd": ".كورس", "ffmpeg": "aecho=0.8:0.9:200:0.2,aecho=0.8:0.9:400:0.15,aecho=0.8:0.9:600:0.1"},
            "tremolo": {"name": "ترمولو", "cmd": ".ترمولو", "ffmpeg": "tremolo=0.1:0.5"},
            "vintage": {"name": "فينتج", "cmd": ".فينتج", "ffmpeg": "aecho=0.8:0.9:500:0.1,volume=0.8,highpass=f=200,lowpass=f=4000"},
            "megaphone": {"name": "مكبر", "cmd": ".مكبر", "ffmpeg": "volume=2.0,aecho=0.8:0.9:500:0.3,highpass=f=300,lowpass=f=3000"},
            "telephone": {"name": "تلفون", "cmd": ".تلفون", "ffmpeg": "highpass=f=300,lowpass=f=3000,volume=1.5"},
            "cave": {"name": "كهف", "cmd": ".كهف", "ffmpeg": "aecho=0.8:0.9:2000:0.5,volume=1.2"},
            "underwater": {"name": "تحت الماء", "cmd": ".تحت الماء", "ffmpeg": "aecho=0.8:0.9:1000:0.3,lowpass=f=1000,volume=0.8"},
            "monster": {"name": "وحش", "cmd": ".وحش", "ffmpeg": "atempo=0.6,asetrate=48000*0.5,aecho=0.8:0.9:1000:0.3"},
            "eight_bit": {"name": "8-بت", "cmd": ".8-بت", "ffmpeg": "acrusher=0.1:0.1:0.1,aecho=0.8:0.9:200:0.1"},
            "drunk": {"name": "سكران", "cmd": ".سكران", "ffmpeg": "aecho=0.8:0.9:500:0.3,aresample=44100,atempo=0.9"},
            "stutter": {"name": "تأتأة", "cmd": ".تأتأة", "ffmpeg": "aecho=0.8:0.9:100:0.2,aecho=0.8:0.9:200:0.15"},
            "muffled": {"name": "مكتوم", "cmd": ".مكتوم", "ffmpeg": "lowpass=f=1000,volume=0.7"},
            "radio": {"name": "راديو", "cmd": ".راديو", "ffmpeg": "highpass=f=200,lowpass=f=4000,aecho=0.8:0.9:300:0.1,volume=1.3"}
        }

        self.effect_keys = list(self.effects.keys())
        self.effect_names = [self.effects[k]["name"] for k in self.effect_keys]
        self.temp_dir = os.path.join(os.getcwd(), "temp_voice")
        os.makedirs(self.temp_dir, exist_ok=True)

        # التأثير المختار للبصمات القادمة.
        self.active_effect = None
        # منع إعادة معالجة البصمة التي أرسلها الكلاس.
        self.processing = False

        self._setup_handlers()
        print(f"[VoiceChanger] تم تهيئة كلاس تغيير الصوت للمستخدم {user_id}")

    def _check_access(self, e):
        return (
            self.source_enabled_func()
            and self.is_my_message_func(e)
            and self.user_id not in banned_users
        )
    ...
    def _apply_ffmpeg_effect(self, input_path, output_path, effect_filter):
        try:
            if not self._check_ffmpeg():
                print("[VoiceChanger] ffmpeg غير موجود في النظام")
                return False
            ...
            if result.returncode != 0:
                print(f"[VoiceChanger] خطأ ffmpeg: {result.stderr}")
                return False

            return os.path.exists(output_path) and os.path.getsize(output_path) > 0
        except Exception as ex:
            print(f"[VoiceChanger] خطأ في تطبيق التأثير: {ex}")
            return False

    async def _process_voice_message(self, e, effect_key=None):
        if self.processing:
            return

        effect_key = effect_key or self.active_effect
        if not effect_key or effect_key not in self.effects:
            return

        # نعالج البصمة نفسها فقط، بدون الحاجة إلى الرد عليها.
        if not getattr(e, "voice", False):
            return
        ...
        try:
            ...
            user_temp = os.path.join(self.temp_dir, str(self.user_id))
            os.makedirs(user_temp, exist_ok=True)

            input_path = os.path.join(user_temp, f"input_{timestamp}.ogg")
            output_path = os.path.join(user_temp, f"output_{timestamp}.ogg")

            await e.download_media(file=input_path)

            if not os.path.exists(input_path) or os.path.getsize(input_path) == 0:
                print("[VoiceChanger] فشل تحميل البصمة")
                return

            status_msg = await self.client.send_message(
                e.chat_id,
                f"🔄 جـاري تـغـيـيـر الـصـوت ↤ {effect['name']}"
            )
            ...
            if not success:
                if status_msg:
                    await self.safe_edit(
                        status_msg,
                        "✧ فـشـل تـغـيـيـر الـصـوت، تـأكـد مـن تـثـبـيـت FFmpeg و libopus"
                    )
                return

            # حذف البصمة الأصلية وإرسال النسخة المعدلة.
            try:
                await e.delete()
            except Exception:
                pass
            ...
            print(f"[VoiceChanger] خطأ في معالجة البصمة: {ex}")
            if status_msg:
                try:
                    await self.safe_edit(status_msg, f"✧ خـطـأ: {str(ex)}")
                except Exception:
                    pass
        finally:
            self.processing = False
    ...
    def _setup_handlers(self):
        # الأمر نفسه كما هو.
        @self.client.on(events.NewMessage(pattern=r'^\.م19$', outgoing=True))
        async def voice_menu(e):
            if not self._check_access(e):
                return

            text = """
⟪───── اوامـر تـغـيـيـر الـصـوت ─────⟫
...
- اخـتـر الـصـوت ثـم ارسـل الـبـصـمـة.

╰─────────────────────────
"""
            await self.safe_edit(e, text)

        # نفس أوامر اختيار الأصوات الموجودة في الكلاس.
        for effect_key, effect_data in self.effects.items():
            cmd_pattern = re.escape(effect_data["cmd"])

            @self.client.on(
                events.NewMessage(
                    pattern=rf'^{cmd_pattern}$',
                    outgoing=True
                )
            )
            async def effect_handler(e, key=effect_key):
                if not self._check_access(e):
                    return
                ...
                await self.safe_edit(
                    e,
                    f"ᯓ『تـم تـفـعـيـل الـصـوت ↤ {self.effects[key]['name']}』ᯓ"
                )

        # أي بصمة يرسلها المستخدم بعد اختيار التأثير يتم تحويلها تلقائياً.
        @self.client.on(events.NewMessage(outgoing=True))
        async def voice_listener(e):
            if not self.source_enabled_func():
                return
            if self.user_id in banned_users:
                return
            if self.processing or not self.active_effect:
                return
            if not getattr(e, "voice", False):
                return

            await self._process_voice_message(e, self.active_effect)
    ...
    async def stop_all(self):
        try:
            ...
            os.makedirs(self.temp_dir, exist_ok=True)
            print("[VoiceChanger] تم إيقاف كلاس تغيير الصوت")
        except Exception as ex:
            print(f"[VoiceChanger] خطأ في الإيقاف: {ex}")
...
• `.م17` ➪ **اوامـر التحويل**
• `.م18` ➪ **اوامـر الـمـسـح**
• `.م19` ➪ **اوامـر تـغـيـيـر الـصـوت**
✦ ────『**جمر واراي يسلمون عليك**』──── ✦
"""
...
│`.ترجمه fr` ↤ لـتـرجـمـه الـرسـالـه الـى الـفـرنـسـاويـه│`.ترجمه es` ↤ لـتـرجـمـه الـرسـالـه الـى الاسـبـانـيـه
...
"""
    ...
    async def stop(self):
        ...
        await self.delete_manager.stop_all()
        await self.voice_changer.stop_all()
        try:
...
async def delete_session_completely(session, user_id):
    ...
    try:
        ...
        await session.delete_manager.stop_all()
        await session.voice_changer.stop_all()
